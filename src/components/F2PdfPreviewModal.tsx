...
import { X, Download, FileText, Image as ImageIcon, Check, ExternalLink, Layers } from 'lucide-react';
import { CompanyHeaderInfo, ExpenseItem } from '../types/expense';
import { generateF2SummaryPdf, generateCompiledBillsPdf, BillSheetSummary } from '../services/pdfGenerator';
...
export const F2PdfPreviewModal: React.FC<F2PdfPreviewModalProps> = ({
  ...
  const [billsSheets, setBillsSheets] = useState<BillSheetSummary[]>([]);
  ...
  useEffect(() => {
    if (isOpen && expenses.length > 0) {
      ...
      Promise.all([
        ...
        setBillsSheets(billsRes.sheets);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md overflow-hidden">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 bg-slate-950 p-4 overflow-hidden relative flex flex-col items-center justify-center">
          {isGenerating ? (
            ...
            <div className="w-full h-full flex flex-col gap-2 min-h-0">
              {/* What the file actually contains — the inline viewer on iPhone
                  shows only the first page, so state it explicitly. */}
              {billsSheets.length > 0 && (
                <div className="shrink-0 rounded-xl border border-slate-800 bg-slate-900 px-3 py-2">
                  <div className="flex items-center gap-1.5 text-[11px] font-bold text-slate-200">
                    <Layers className="w-3.5 h-3.5 text-cyan-400" />
                    <span>
                      {billsSheets.length} sheet{billsSheets.length === 1 ? '' : 's'} — one per expense category
                    </span>
                  </div>

                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {billsSheets.map((sheet, i) => (
                      <span
                        key={`${sheet.category}-${i}`}
                        className="px-2 py-0.5 rounded-lg bg-slate-800 border border-slate-700 text-[10px] text-slate-300"
                      >
                        <span className="font-semibold text-slate-200">{sheet.category}</span>
                        {' · '}
                        {sheet.bills} bill{sheet.bills === 1 ? '' : 's'}
                        {' · SAR '}
                  ...
                  {billsSheets.length > 1 && (
                    <p className="mt-1.5 text-[10px] text-amber-300/90">
                      iPhone previews only show page 1 here — open in a new tab or download to see every sheet.
                    </p>
                  )}
                </div>
              )}
              ...
              <iframe
                src={billsPdfUrl}
                className="w-full flex-1 min-h-0 rounded-xl border border-slate-800 shadow-inner bg-slate-900"
        ...
        <div className="px-6 py-3 border-t border-slate-800 bg-slate-850 flex items-center justify-between text-xs">
          ...
          <div className="flex items-center space-x-3">
            {/* iOS renders only page 1 of a PDF inside an iframe; a real tab shows all sheets. */}
            {(activeTab === 'f2' ? f2PdfUrl : billsPdfUrl) && (
              <a
                href={(activeTab === 'f2' ? f2PdfUrl : billsPdfUrl) as string}
                target="_blank"
                rel="noopener noreferrer"
                className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold transition flex items-center gap-2 border border-slate-700"
              >
